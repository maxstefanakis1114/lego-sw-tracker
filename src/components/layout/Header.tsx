import { Swords, Cloud, CloudOff } from 'lucide-react';
...
export function Header({ syncConnected, onSyncClick }: HeaderProps) {
  return (
    <header className="bg-gradient-to-b from-sw-dark/95 to-sw-black/95 backdrop-blur-md sticky top-0 z-40">
      <div className="max-w-7xl mx-auto px-4 py-4 flex items-center gap-3">
        ...
        <h1 className="text-xl font-black font-[--font-sw-title] tracking-wider flex-1">
        ...
        {onSyncClick && (
          <button
            onClick={onSyncClick}
            className="p-2 rounded-lg hover:bg-sw-border/50 transition-colors cursor-pointer"
            title="Sync"
          >
            {syncConnected ? (
              <Cloud size={20} className="text-sw-green" />
            ) : (
              <CloudOff size={20} className="text-sw-text-dim" />
            )}
          </button>
        )}
      </div>
